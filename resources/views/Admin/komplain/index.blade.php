            ...
            @forelse ($complaints as $c)
                @php
                    ...
                    $prioClass = $prio === 'Tinggi'
                        ...
                        : ($prio === 'Sedang' ? 'bg-success/10 text-success border-success/20' : 'bg-surface-container-high text-on-surface-variant border-outline-variant');
                    $kodeComplain = 'KL-' . str_pad((string) $c->complaint_id, 10, '0', STR_PAD_LEFT);
                @endphp
                <article data-reveal data-komplain-row data-status="{{ $key }}"
                    data-open-id="{{ $c->complaint_id }}"
                    data-open-subjek="{{ $c->subjek ?? $c->kategori }}"
                    data-open-kode="{{ $kodeComplain }}"
                    data-open-statuslabel="{{ $statusLabel }}"
                    data-open-done="{{ in_array($c->status, [\App\Models\Complaint::STATUS_SELESAI, \App\Models\Complaint::STATUS_DITUTUP], true) ? '1' : '0' }}"
                    class="bg-surface-container-lowest border {{ $key === 'open' ? 'border-error/25' : 'border-muted-border' }} rounded-lg p-5 md:p-6 card-premium">
...
<script>
    ...
    function openChatFromCard(el) {
        const card = el.closest('[data-komplain-row]');
        if (!card) return;
        openChatModal(
    ...
    function openChatModal(id, subjek, kode, statusLabel, done) {
        ...
        statusEl.className = 'shrink-0 inline-flex items-center px-2 py-1 rounded-full text-[10px] font-bold uppercase border ' + (done ? 'bg-surface-container-high text-on-surface-variant border-outline-variant' : 'bg-gold-accent/10 text-gold-accent border-gold-accent/30');
        ...
        document.getElementById('chat-more-item-eskalasi').classList.toggle('hidden', !!done);
    ...
    async function loadMessages() {
        ...
        try {
            const url = '{{ route('admin.komplain.messages', ':id:') }}'.replace(':id:', currentChat.id);
    ...
    async function deleteSelectedMessages() {
        ...
        for (const id of ids) {
            try {
                const url = '{{ route('admin.komplain.messages.destroy', [':cid:', ':mid:']) }}'.replace(':cid:', currentChat.id).replace(':mid:', id);
                const resp = await fetch(url, {
                    method: 'DELETE',
                    headers: {
    ...
    async function saveEditMessage() {
        ...
        try {
            const url = '{{ route('admin.komplain.messages.update', [':cid:', ':mid:']) }}'.replace(':cid:', currentChat.id).replace(':mid:', id);
            const resp = await fetch(url, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
    ...
    async function sendMessage() {
        ...
        try {
            const url = '{{ route('admin.komplain.messages.store', ':id:') }}'.replace(':id:', currentChat.id);
            const resp = await fetch(url, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (openId) {
            var row = document.querySelector('[data-open-id="' + openId + '"]');
            if (row) openChatFromCard(row);
        }
    });
</script>
@endpush
...
<script>
    ...
    document.getElementById('confirm-komplain-submit')?.addEventListener('click', function () {
        if (!_eskalasiId) return;
        const btn = this;
        btn.disabled = true;
        const url = '{{ route('admin.komplain.eskalasi', ':id:') }}'.replace(':id:', _eskalasiId);
        fetch(url, {
            method: 'POST',
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
