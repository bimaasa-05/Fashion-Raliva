...
<nav class="md:hidden fixed bottom-0 inset-x-0 z-50 flex justify-around items-center h-[72px] bg-[var(--chrome-bg)] text-[var(--chrome-text)] px-xs pb-safe border-t border-[var(--chrome-border)] shadow-sm">
    @php
        $bnItems = [
            ...
            ['label' => __('Pesanan'), 'icon' => 'receipt_long', 'href' => auth()->check() ? route('customer.orders') : route('customer.cek-resi'), 'active' => request()->routeIs('customer.orders') || request()->routeIs('customer.order-tracking') || request()->routeIs('customer.cek-resi') || request()->routeIs('customer.cek-resi.search')],
