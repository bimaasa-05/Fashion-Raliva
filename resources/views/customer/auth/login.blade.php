...
<head>
    ...
    <link
        ...
        rel="stylesheet" />
    <style>
        /* Global Hide Scrollbar — customer auth */
        ::-webkit-scrollbar {
    ...
    <style>
        ...
        html.theme-dark .bg-error-container {
        ...
        :root {
            --gold-inner: rgba(139, 30, 30, .30);
        }

        html.theme-dark {
            --gold-inner: rgba(139, 30, 30, .25);
        }
        ...
        .atl-chip:hover {
            box-shadow: 0 6px 16px -4px rgba(139, 30, 30, .28);
        }
        ...
        html.theme-dark .atl-blade-a,
        html.theme-dark .atl-blade-b {
            stroke: #ffffff;
            stroke-opacity: .15;
        }

        html.theme-dark .atl-pivot {
            fill: #ffffff;
            fill-opacity: .15;
        }

        html.theme-dark .atl-ring {
            stroke: #ffffff;
            stroke-opacity: .15;
        }

        html.theme-dark .atl-rim {
            stroke: #ffffff;
            stroke-opacity: .12;
        }

        html.theme-dark .atl-dots circle {
            fill: #ffffff;
            fill-opacity: .18;
        }

        html.theme-dark .atl-shaft {
            stroke: #ffffff;
            stroke-opacity: .15;
        }

        html.theme-dark .atl-eye {
            stroke: #ffffff;
            stroke-opacity: .14;
        }

        html.theme-dark .atl-thread {
            stroke: #ffffff;
            stroke-opacity: .14;
        }

        html.theme-dark .atl-threadflow {
            stroke: #A32626;
            stroke-opacity: .45;
        }
        ...
        #remember:focus-visible {
            box-shadow: 0 0 0 3px rgba(139, 30, 30, .3);
        }
...
<body class="bg-surface text-on-surface antialiased font-body-lg min-h-screen flex flex-col lg:flex-row">
    ...
    <aside class="relative overflow-hidden shrink-0 h-44 lg:h-auto lg:w-[44%] flex">
        <img alt="RALIVA Login Editorial 1" src="{{ asset('assets/picture/login-pictures/1.jfif') }}"
            class="atl-slide absolute inset-0 w-full h-full object-cover transition-opacity duration-1000" />
        <img alt="RALIVA Login Editorial 2" src="{{ asset('assets/picture/login-pictures/2.jfif') }}"
            class="atl-slide absolute inset-0 w-full h-full object-cover opacity-0 transition-opacity duration-1000" />
        <img alt="RALIVA Login Editorial 3" src="{{ asset('assets/picture/login-pictures/3.jfif') }}"
            class="atl-slide absolute inset-0 w-full h-full object-cover opacity-0 transition-opacity duration-1000" />
        <div class="absolute inset-0 bg-black/50"></div>
        <div
            class="relative z-10 flex flex-col items-center justify-center text-center h-full w-full p-lg lg:items-start lg:justify-between lg:text-left lg:p-xl">
            <div>
                <span class="font-display-lg text-headline-lg tracking-widest text-white">RALIVA</span>
                <p class="font-label-sm text-label-sm text-white/80 tracking-wide mt-1 lg:hidden">
                    {{ __('The Art of Everyday Dressing') }}</p>
    ...
    <main
        ...
        <div class="fixed top-sm right-sm z-50 flex items-center gap-xs">
            ...
            <button id="theme-toggle-btn" aria-label="Toggle theme" title="Mode gelap" type="button"
                ...
                <span class="relative block h-5 w-5">
                    <svg class="atl-ic-sun absolute inset-0 h-5 w-5" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="1.5" stroke-linecap="round">
                        <circle cx="12" cy="12" r="4" />
                        <path
                            d="M12 3v1.8M12 19.2V21M3 12h1.8M19.2 12H21M5.64 5.64l1.27 1.27M17.09 17.09l1.27 1.27M18.36 5.64l-1.27 1.27M6.91 17.09l-1.27 1.27" />
                    </svg>
                    <svg class="atl-ic-moon absolute inset-0 h-5 w-5" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79Z" />
                        <circle cx="17.5" cy="5.5" r=".75" fill="currentColor" stroke="none" />
                        <circle cx="20.5" cy="9.5" r=".55" fill="currentColor" stroke="none" />
                    </svg>
                </span>
            </button>
        </div>
        ...
        <svg class="atl-float atl-scissor" viewBox="0 0 32 20" aria-hidden="true">
            <g class="atl-blade-a">
                ...
                <path d="M8.6 5.2 29 13.4" />
            </g>
            <g class="atl-blade-b">
                <circle cx="6" cy="16" r="3" />
                <path d="M8.6 14.8 29 6.6" />
            </g>
        ...
        <svg class="atl-float atl-btn" viewBox="0 0 20 20" aria-hidden="true">
            ...
            <g class="atl-dots">
                ...
                <circle cx="12.8" cy="7.2" r="1.1" />
                <circle cx="7.2" cy="12.8" r="1.1" />
        ...
        <svg class="atl-float atl-needle" viewBox="0 0 44 44" aria-hidden="true">
            ...
            <ellipse class="atl-eye" cx="12" cy="28.2" rx="3.2" ry="1.9"
                transform="rotate(42 12 28.2)" />
        ...
        <div class="flex-grow flex flex-col justify-center w-full max-w-md mx-auto py-xl relative z-10">
            <div class="border-2 border-secondary rounded-lg p-lg lg:p-xl frame-gold">
                <!-- Heading -->
                <div class="mb-md">
                    <p class="font-label-caps text-label-caps uppercase tracking-widest text-secondary mb-xs">
                        {{ __('Raliva Account') }}</p>
                    <h2 class="font-headline-lg-mobile text-headline-lg-mobile text-on-surface mb-xs">
                        {{ __('Welcome Back') }}</h2>
                </div>
                @auth
                    <div
                        ...
                        . {{ __('Masuk sebagai akun lain akan mengganti sesi ini.') }}
                        <form method="POST" action="{{ route('logout') }}" class="inline">
                            ...
                            @endif
                            <button type="submit"
                                class="text-secondary font-semibold hover:opacity-80 underline ml-1">{{ __('Keluar') }}</button>
                        </form>
                    </div>
                @endauth
                @if (session('success'))
                ...
                <form id="login-form" novalidate method="POST" action="{{ route('login') }}">
                    ...
                    <div class="mb-sm">
                        ...
                        <p class="hidden font-label-sm text-label-sm text-error mt-xs" id="password-error">
                            {{ __('Password is required.') }}</p>
                    </div>
                    <!-- Remember Me + Forgot Password -->
                    <div class="flex items-center justify-between">
                        <label class="flex items-center gap-sm cursor-pointer">
                            <input class="w-4 h-4 shrink-0 cursor-pointer" id="remember" name="remember"
                                type="checkbox" />
                            <span
                                class="font-label-sm text-label-sm text-on-surface-variant">{{ __('Remember Me') }}</span>
                        </label>
                        <div class="flex justify-end">
                            <a class="font-label-sm text-label-sm text-secondary hover:opacity-80 transition-opacity"
                    ...
                    <button
                        ...
                        <span class="material-symbols-outlined text-[20px] animate-spin hidden"
                            id="login-spinner">progress_activity</span>
                    </button>
                </form>
                <!-- Google Divider -->
                <div class="flex items-center gap-sm my-sm">
                    ...
                    <span
                        class="font-label-caps text-label-caps uppercase tracking-widest text-outline">{{ __('or') }}</span>
                    <span class="h-px flex-grow bg-outline-variant"></span>
                </div>
                <!-- Sign In with Google -->
                ...
                <p class="text-center font-body-sm text-body-sm text-on-surface-variant mt-lg">
                    ...
                    <a class="text-secondary font-semibold hover:opacity-80 transition-opacity ml-1"
                        id="register-link" href="{{ route('register') }}">{{ __('REGISTER') }}</a>
                </p>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        window.addEventListener('pageshow', function(e) {
            if (e.persisted) {
                document.documentElement.classList.toggle('theme-dark', localStorage.getItem('raliva-theme') ===
                    'dark');
                updateThemeChipTitle();
            }
        });
