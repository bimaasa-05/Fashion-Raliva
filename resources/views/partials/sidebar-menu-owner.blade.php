            ...
            <div class="space-y-1 {{ $collapsible && !$isActive ? 'hidden' : '' }}" @if ($collapsible) data-sidebar-group @endif>
                @foreach ($group['items'] as $item)
                    <a class="group flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200
                        ...
                        <span class="material-symbols-outlined text-[20px] @if(request()->routeIs($item['route'])) fill text-gold-accent @else text-white/70 group-hover:text-white transition-colors @endif">
                            {{ $item['icon'] }}
                        </span>
