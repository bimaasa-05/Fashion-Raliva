<div class="relative" data-profile-container>
    <button type="button" data-profile-toggle class="flex items-center gap-3 hover:opacity-80 transition-opacity" aria-label="Menu profil">
        @if (empty($compact))
            <div class="text-right hidden lg:block">
                <p class="font-label-sm text-sm text-on-surface leading-tight">Rizky Pratama</p>
                <p class="text-on-surface-variant text-xs">Super Admin</p>
            </div>
        @endif
    ...
    <div data-profile-menu class="hidden absolute right-0 top-full mt-2 w-60 bg-surface-container-lowest border border-muted-border rounded-lg shadow-xl z-[60] overflow-hidden">
        <div class="px-4 py-3 border-b border-muted-border">
            <p class="font-label-sm text-sm text-on-surface">Rizky Pratama</p>
            <p class="text-on-surface-variant text-xs mt-0.5">Super Admin</p>
        </div>
        <a href="{{ route('superadmin.profil') }}" class="flex items-center gap-3 px-4 py-3 text-on-surface hover:bg-surface-container-low transition-colors">
        ...
        <a href="{{ route('superadmin.pengaturan-sistem') }}" class="flex items-center gap-3 px-4 py-3 text-on-surface hover:bg-surface-container-low transition-colors">
