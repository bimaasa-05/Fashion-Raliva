...
@php
    $pnama = $user->nama_lengkap ?? 'Produksi';
...
<div data-real class="hidden space-y-section-gap">
    <div class="w-full grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
        ...
        <section class="relative overflow-hidden bg-surface-container-lowest border border-muted-border rounded-xl card-premium hero-glow profil-hero rise">
            <span class="material-symbols-outlined fill absolute -right-6 -bottom-10 text-[220px] text-gold-accent/[0.06] pointer-events-none select-none" aria-hidden="true">cut</span>
...
<div class="avatar-ring">
                            ...
                            @if ($user->foto_profil_url)
                                ...
                                <span id="hero-avatar-initial" style="display:none">{{ $pinit }}</span>
                            @else
                                <span id="hero-avatar-initial">{{ $pinit }}</span>
                            @endif
                            </div>
                        </div>
        ...
        <section class="rise rise-d3">
            <div class="bg-surface-container-lowest border border-muted-border rounded-xl card-premium profil-card p-6 md:p-8 relative overflow-hidden">
                ...
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        <h3 class="font-title-md text-title-md text-on-surface premium-heading">Role &amp; Workshop</h3>
                        <p class="text-on-surface-variant font-body-md text-sm">Penugasan pada akun Anda.</p>
                    </div>
                </div>
                    ...
                    <div class="profil-row flex justify-between gap-4 pb-4 items-start">
                        ...
                        <dd>
                            <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-surface-container-high text-on-surface text-xs font-bold border border-outline-variant whitespace-nowrap">
                                <span class="material-symbols-outlined text-[16px] text-gold-accent">cut</span>
                                Atelier Produksi Raliva
                            </span>
                        </dd>
                    </div>
                </dl>
        ...
        <section class="rise rise-d1">
            <div class="bg-surface-container-lowest border border-muted-border rounded-xl card-premium profil-card p-6 md:p-8">
                ...
                <form method="POST" action="{{ route('produksi.profil.update') }}" id="profil-form" class="space-y-5" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                            <label class="block font-label-sm text-label-sm text-on-surface-variant uppercase mb-2">Workshop</label>
                            <input class="w-full bg-surface-container border border-muted-border rounded-lg p-4 font-body-md text-body-md text-on-surface-variant cursor-not-allowed" type="text" value="Atelier Produksi Raliva" disabled />
                        </div>
                    </div>
                    @if($user->foto_profil_url)
...
<div class="w-10 h-10 rounded-lg icon-tile flex items-center justify-center shrink-0">
                ...
                <form method="POST" action="{{ route('produksi.profil.password') }}" id="password-form" class="space-y-5">
                    @csrf
                    @method('PUT')
                    <div>
