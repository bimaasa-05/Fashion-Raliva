    ...
    <section class="bg-surface-container-lowest border border-muted-border rounded-lg p-4 md:p-6 card-premium">
        <div class="bg-surface-container-low border border-muted-border rounded-lg p-4 mb-6">
            ...
            <div class="flex flex-wrap gap-gutter">
                <select data-table-filter="kategori" aria-label="Filter kategori" class="bg-surface-container-lowest border border-muted-border rounded-lg px-3 py-2.5 font-body-md text-sm text-on-surface focus:outline-none focus:border-gold-accent">
                ...
                <select data-table-filter="status" aria-label="Filter status stok" class="bg-surface-container-lowest border border-muted-border rounded-lg px-3 py-2.5 font-body-md text-sm text-on-surface focus:outline-none focus:border-gold-accent">
                ...
                <select aria-label="Urutkan" class="bg-surface-container-lowest border border-muted-border rounded-lg px-3 py-2.5 font-body-md text-sm text-on-surface focus:outline-none focus:border-gold-accent">
            ...
            <table class="w-full min-w-[900px] premium-table">
                ...
                <tbody class="font-body-md text-sm">
                    @foreach ($products as $product)
                        @php
                            $statusKey = strtolower($product[6]);
        ...
        <div data-pagination class="flex flex-wrap items-center justify-between gap-4 mt-6">
            <p class="font-label-sm text-xs text-on-surface-variant">Menampilkan 1–{{ count($products) }} dari {{ count($products) }} produk • Gudang Utama Bandung</p>
            ...
            <div class="relative mx-auto mt-16 md:mt-24 w-[calc(100%-2rem)] max-w-lg bg-surface-container-lowest border border-muted-border rounded-xl border-t-4 border-t-gold-accent/70 shadow-xl p-6 max-h-[80vh] overflow-y-auto">
                <div class="flex items-start justify-between gap-4 mb-6">
                    <div>
                        ...
                        <p class="text-on-surface-variant font-label-sm text-xs uppercase tracking-wider mt-1">SKU {{ $product[1] }} • {{ $product[2] }}</p>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-gutter mb-6">
                    <div class="bg-surface-container-low border border-muted-border rounded-lg p-4">
                        <p class="font-label-sm text-[10px] uppercase tracking-widest text-on-surface-variant">Stok Tersedia</p>
                        <p class="font-headline-lg-mobile text-headline-lg-mobile {{ in_array($product[6], ['Kritis', 'Habis']) ? 'text-error' : 'text-on-surface' }}">{{ $product[4] }}</p>
                    </div>
                    <div class="bg-surface-container-low border border-muted-border rounded-lg p-4">
                ...
                <dl class="space-y-4 font-body-md text-sm">
                    ...
                    <div class="flex justify-between gap-4">
                        ...
                        <dd class="text-on-surface">{{ $product[7] }}</dd>
                    </div>
                </dl>
                <div class="flex gap-gutter mt-6">
                    <a href="{{ route('gudang.riwayat-stok') }}" class="flex-1 text-center py-3 border border-muted-border rounded-lg font-label-sm text-[11px] text-gold-accent uppercase tracking-widest hover:bg-gold-accent/10 hover:border-gold-accent/40 transition-colors">Lihat Riwayat</a>
                    <button type="button" data-modal-close class="flex-1 py-3 bg-deep-onyx text-on-primary font-label-sm text-[11px] uppercase tracking-widest rounded btn-premium">Tutup</button>
                </div>
            </div>
        </div>
    @endforeach
