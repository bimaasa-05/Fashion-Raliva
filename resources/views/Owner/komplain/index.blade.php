...
<script>
    ...
    async function loadMessages() {
        ...
        try {
            const url = '{{ route('owner.komplain.messages', ':id:') }}'.replace(':id:', currentChat.id);
    ...
    async function deleteSelectedMessages() {
        ...
        for (const id of ids) {
            try {
                const url = '{{ route('owner.komplain.messages.destroy', [':cid:', ':mid:']) }}'.replace(':cid:', currentChat.id).replace(':mid:', id);
                const resp = await fetch(url, {
                    method: 'DELETE',
                    headers: {
    ...
    async function saveEditMessage() {
        ...
        try {
            const url = '{{ route('owner.komplain.messages.update', [':cid:', ':mid:']) }}'.replace(':cid:', currentChat.id).replace(':mid:', id);
            const resp = await fetch(url, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
    ...
    async function sendMessage() {
        ...
        try {
            const url = '{{ route('owner.komplain.messages.store', ':id:') }}'.replace(':id:', currentChat.id);
            const resp = await fetch(url, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
