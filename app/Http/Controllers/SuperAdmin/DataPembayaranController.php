...
class DataPembayaranController extends Controller
{
    public function index(Request $request)
    {
        ...
        $stats = [
            'semua' => Payment::count(),
            'terverifikasi' => Payment::where('status', Payment::STATUS_TERVERIFIKASI)->count(),
            'menunggu_verifikasi' => Payment::where('status', Payment::STATUS_MENUNGGU_VERIFIKASI)->count(),
            'pending' => Payment::where('status', Payment::STATUS_PENDING)->count(),
            'ditolak' => Payment::where('status', Payment::STATUS_DITOLAK)->count(),
            'kadaluarsa' => Payment::where('status', Payment::STATUS_KADALUARSA)->count(),
        ];
