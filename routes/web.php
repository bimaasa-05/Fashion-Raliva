...
Route::post('/reset-password', [ResetPasswordController::class, 'reset'])->middleware('throttle:6,1')->name('password.update');
...
Route::prefix('superadmin')->name('superadmin.')->middleware(['auth', 'role:Super Admin'])->group(function () {
    ...
    Route::get('/data-bank/{bank}/edit', [DataBankController::class, 'edit'])->name('data-bank.edit');
    Route::get('/data-bank/account/{account}/edit', [DataBankController::class, 'editAccount'])->name('data-bank.account.edit');
...
Route::prefix('admin')->name('admin.')->middleware(['auth', 'role:Admin', 'store-active'])->group(function () {
    ...
    Route::post('/slot/paket/{paket}', [\App\Http\Controllers\Admin\SlotController::class, 'beliPaket'])->name('slot.paket.beli');
    ...
    Route::put('/kurir/courier/{kurir}', [\App\Http\Controllers\Admin\KurirController::class, 'updateCourier'])->name('kurir.courier.update');
    Route::delete('/kurir/courier/{kurir}', [\App\Http\Controllers\Admin\KurirController::class, 'destroyCourier'])->name('kurir.courier.destroy');
    Route::post('/kurir/layanan', [\App\Http\Controllers\Admin\KurirController::class, 'storeLayanan'])->name('kurir.layanan.store');
    Route::put('/kurir/layanan/{layanan}', [\App\Http\Controllers\Admin\KurirController::class, 'updateLayanan'])->name('kurir.layanan.update');
    Route::delete('/kurir/layanan/{layanan}', [\App\Http\Controllers\Admin\KurirController::class, 'destroyLayanan'])->name('kurir.layanan.destroy');
...
Route::prefix('owner')->name('owner.')->middleware(['auth', 'role:Owner', 'store-active'])->group(function () {
    ...
    Route::post('/keuangan/pencairan', [SaldoController::class, 'storePencairan'])->name('keuangan.pencairan.store');
...
Route::prefix('produksi')->name('produksi.')->middleware(['auth', 'role:Produksi', 'store-active'])->group(function () {
    ...
    Route::post('/data-produksi/{order}/bahan', [ProduksiDataController::class, 'storeBahan'])->name('data-produksi.bahan');
    Route::post('/data-produksi/backfill-bahan', [ProduksiDataController::class, 'backfillBahan'])->name('data-produksi.backfill-bahan');
    ...
    Route::match(['put', 'post'], '/profil', [ProduksiProfilController::class, 'updateProfile'])->name('profil.update');
    Route::match(['put', 'post'], '/profil/password', [ProduksiProfilController::class, 'updatePassword'])->name('profil.password');
    // Permintaan Operasional (ajukan ke Admin)
